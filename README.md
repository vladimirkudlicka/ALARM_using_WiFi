...
If the connection is lost the onboard LED will reamin turned on.
When the button is pressed the LED starts blinking and it sends a message to the alarm speaker (second microcontroler) using socket server.
Alarm (microcontroler 2):
This microc0ontroler is the server.
This microcontroler is connected to an active buzzer using a PNP trasistor, there is a restor between the pin and the base with value 2.7k ohms 
...
You can turn off the alarm using the button on the microcontroler 2. The green LED on microcontroler one also stops blinking.
...
If there is no connection check in 500s microcontroler 2 will restart.
